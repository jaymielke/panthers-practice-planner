...
export default function PracticePlanner() {
  ...
  function addDrill() {
    if (!drillName) return;
    const newDrill = { id: Date.now(), name: drillName, notes: drillNotes, video: drillVideo, category: drillCategory };
    setDrills([...drills, newDrill]);
    setDrillName(""); setDrillNotes(""); setDrillVideo("");
  }
  ...
  if (currentPage === "home") {
    return (
      <div style={{ padding: "20px", maxWidth: "700px", margin: "auto", fontFamily: "Arial", textAlign: "center", backgroundColor: COLORS.navy, color: COLORS.white, minHeight: "100vh" }}>
        <img src="/panthers-logo.png" alt="Panthers Logo" style={{ width: "120px", marginBottom: "20px" }} />
        <h1 style={{ color: COLORS.gold }}>Kitchener Panthers U8 Practice Planner ⚾</h1>
  ...
  if (currentPage === "addDrill") {
    return (
      <div style={{ padding: "20px", maxWidth: "700px", margin: "auto", fontFamily: "Arial", minHeight: "100vh" }}>
        ...
        <br /><br />
        <textarea placeholder="Drill Instructions" value={drillNotes} onChange={e=>setDrillNotes(e.target.value)} style={{ width: "100%", padding: "8px" }} />
        <br /><br />
        <input placeholder="Video link" value={drillVideo} onChange={e=>setDrillVideo(e.target.value)} style={{ width: "100%", padding: "8px" }} />
        <br /><br />
        <button onClick={addDrill} style={{ padding: "12px 20px", borderRadius:"8px", backgroundColor: COLORS.gold, color: COLORS.navy, fontWeight:"bold", border:"none" }}>Save Drill</button>

        <h2>Existing Drills</h2>
        {drills.map(d => (
          <div key={d.id} style={{ border: "1px solid #ccc", borderRadius:"8px", padding:"10px", marginBottom:"10px" }}>
  ...
  if (currentPage === "createPractice") {
    return (
      <div style={{ padding: "20px", maxWidth: "700px", margin: "auto", fontFamily: "Arial", minHeight: "100vh" }}>
        ...
        <br />
        <button onClick={savePractice} style={{ padding: "12px 20px", borderRadius:"8px", backgroundColor: COLORS.gold, color: COLORS.navy, fontWeight:"bold", border:"none" }}>Save Practice Plan</button>
      </div>
    );
  }
    ...
    return (
      <div style={{ padding: "20px", maxWidth: "700px", margin: "auto", fontFamily: "Arial", minHeight: "100vh" }}>
        ...
        {practicesForDate.map(p=>(
          <button key={p.id} onClick={()=>setSelectedPracticeId(p.id)} style={{ display:"block", margin:"5px 0", padding:"8px", width:"100%" }}>
            Practice at {p.date}
          </button>
        ))}
        {selectedPractice && (
