...
const Icons = {
  ...
  copy:     "M8 4H6a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-4-4H8zM14 2v6h6M10 11h4M10 15h4M10 7h1",
};
...
function VenueChip({ venue, small = false }) {
  ...
  return (
    <span style={{
      ...
      <Ico name={VENUE_ICONS[venue]||"star"} size={10}/>{venue}
    </span>
  );
}
...
function makeTheme(dark) {
  return dark ? {
    ...
    inputBorder:"rgba(95,141,181,0.22)",
    navBg:      "#111827",
    navBorder:  "rgba(95,141,181,0.2)",
    navText:    "rgba(255,255,255,0.3)",
    pillBorder: "rgba(255,255,255,0.1)",
    pillText:   "rgba(255,255,255,0.4)",
    playerChip: "rgba(255,255,255,0.07)",
    playerText: "rgba(255,255,255,0.5)",
    noteText:   "rgba(255,255,255,0.5)",
    toastBg:    "#1a2540",
    modalBg:    "#111827",
    overlayBg:  "rgba(6,10,20,0.78)",
    danger:     "#e05252",
    success:    "#3dba7a",
    recentCol:  "#ffc400",
    isDark:     true,
    ...
    surface:    "#ffffff",
    surfaceUp:  "#e8f0f8",
    border:     "rgba(95,141,181,0.2)",
    borderHi:   "rgba(95,141,181,0.5)",
    steel:      "#3d6a94",
    steelDim:   "rgba(61,106,148,0.08)",
    steelLight: "#1a4a8a",
    text:       "#111827",
    textMuted:  "rgba(17,24,39,0.55)",
    textDim:    "rgba(17,24,39,0.35)",
    inputBg:    "#ffffff",
    inputBorder:"rgba(95,141,181,0.35)",
    navBg:      "#ffffff",
    navBorder:  "rgba(95,141,181,0.2)",
    navText:    "rgba(17,24,39,0.35)",
    pillBorder: "rgba(17,24,39,0.15)",
    pillText:   "rgba(17,24,39,0.45)",
    playerChip: "rgba(17,24,39,0.07)",
    playerText: "rgba(17,24,39,0.55)",
    noteText:   "rgba(17,24,39,0.55)",
    toastBg:    "#1a4a8a",
    modalBg:    "#ffffff",
    overlayBg:  "rgba(17,24,39,0.55)",
    danger:     "#c0392b",
    success:    "#1a7a4a",
    recentCol:  "#c98f00",
    isDark:     false,
...
.filter-pill{display:inline-flex;align-items:center;gap:5px;padding:6px 13px;border-radius:20px;border:1.5px solid ${T.pillBorder};background:transparent;cursor:pointer;white-space:nowrap;font-family:'Source Sans 3',sans-serif;font-size:12px;font-weight:600;color:${T.pillText};transition:all 0.15s;-webkit-tap-highlight-color:transparent;}
...
/* Pick items (create) */
.pick-item{background:${T.surface};border:1.5px solid ${T.border};border-radius:11px;padding:13px 15px;margin-bottom:9px;display:flex;align-items:center;gap:12px;cursor:pointer;transition:all 0.15s;user-select:none;-webkit-tap-highlight-color:transparent;}
...
.pick-name{font-weight:600;font-size:15px;color:${T.text};margin-bottom:5px;}
...
/* Plan expanded drill list */
.plan-drills{margin-top:4px;}
.plan-drill-row{padding:12px 0;border-bottom:1px solid ${T.border};}
.plan-drill-row:last-child{border-bottom:none;}
.plan-drill-time-row{display:flex;align-items:center;gap:10px;margin-bottom:8px;}
...
/* Share blocks styled to match drill library */
...
@media(max-width:400px){.mv-date{font-size:17px;}.mv-timer-digits{font-size:26px;}}
...
function useCountdown({ totalSeconds, running, onExpire }) {
  ...
  useEffect(() => {
    ...
    function tick() {
      ...
      if (remaining <= 0) { if (!expiredRef.current) { expiredRef.current=true; onExpire&&onExpire(); } return; }
...
export default function PracticePlanner() {
  ...
  const [shared] = useState(()=>{
    try{const p=new URLSearchParams(window.location.search).get("p");return p?decodePlan(p):null;}catch{return null;}
  });
  ...
  function savePractice(){
    ...
    setPicked([]);setPDate(today);setPTime("17:00");toast.show("Practice saved");setTab("plans");
  }
  ...
  function saveEditPlan(){if(!ePicked.length)return toast.show("Pick at least one drill");setPlans(prev=>prev.map(p=>p.id===editPlan.id?{...p,date:ePDate,start:ePTime,drills:ePicked}:p));setEditPlan(null);toast.show("Practice updated");}
  ...
  const applyFilter  = (list,f)=>f==="All"?list:list.filter(d=>d.category===f);
  const filteredDrills = applyFilter(drills,drillFilter);

  function DrillCard({d,showActions=true,compact=false}) {
    const c=CAT[d.category]||CAT["Hitting"];
    return (
      <div className="drill-item" style={{borderLeftColor:c.border,borderLeftWidth:3,marginBottom:compact?8:10}}>
        <div className="drill-item-header">
          <div style={{flex:1}}>
            <div className={compact?"drill-name-sm":"drill-name"}>{d.name}</div>
            <div className="meta-chips">
              <CatChip cat={d.category} small={compact}/>
            ...
            {d.notes&&(
              <ul className="drill-notes">
                {d.notes.split("\n").filter(Boolean).slice(0,compact?2:3).map((n,i)=>(
                  <li key={i}><span style={{color:c.text}}>· </span><span style={{color:T.noteText}}>{n}</span></li>
                ))}
              </ul>
            )}
            {d.video&&(
  ...
  return (
    <div className="app">
      ...
      <div className="top-bar">
        ...
        <button className="theme-btn" onClick={()=>setDark(d=>!d)} title="Toggle light/dark">
      ...
      <div className="scroll-area">
        ...
        {tab==="drills"&&(
          <>
            ...
            {!showForm&&(
              <button className="btn btn-primary btn-full" style={{marginBottom:14}} onClick={()=>{resetForm();setShowForm(true);}}>
            ...
            {showForm&&(
              <div className="card" style={{marginBottom:14}}>
                ...
                <div className="row2" style={{marginBottom:14}}>
                  ...
                  <div><label className="label">Players</label>
                ...
                <div style={{marginBottom:14,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                  <CatChip cat={dCat}/>{dVenue!=="Both"&&<VenueChip venue={dVenue}/>}
            ...
            ):filteredDrills.map(d=><DrillCard key={d.id} d={d} showActions={true}/>)}
          </>
        )}
        ...
        {tab==="create"&&(
          <>
            ...
            {drills.length===0?(
              ...
              <>
                {applyFilter(recentDrills,createFilter).length>0&&(
                  <>
                    <div className="recent-label"><Ico name="star" size={12}/> Recently Used</div>
                    {applyFilter(recentDrills,createFilter).map(d=><PickItem key={d.id} d={d} sel={!!picked.find(p=>p.id===d.id)} idx={picked.findIndex(p=>p.id===d.id)} onToggle={togglePick}/>)}
                    {applyFilter(otherDrills,createFilter).length>0&&<div className="divider-label" style={{marginTop:12}}>All Drills</div>}
                  </>
                )}
                {applyFilter(otherDrills,createFilter).map(d=><PickItem key={d.id} d={d} sel={!!picked.find(p=>p.id===d.id)} idx={picked.findIndex(p=>p.id===d.id)} onToggle={togglePick}/>)}
                {applyFilter(recentDrills,createFilter).length===0&&applyFilter(otherDrills,createFilter).length===0&&(
                  <div className="empty" style={{padding:"24px 0"}}><Ico name="filter" size={28}/><p>No {createFilter} drills found.</p></div>
                )}
              </>
            )}
            {picked.length>0&&(
                  ...
                  <div className="plan-drills">
                    <div className="plan-drill-row">
                      <div className="plan-drill-time-row">
                        ...
                        <div className="plan-drill-time">{schedule[0].start} – {schedule[0].end}</div>
                        <div className="plan-drill-name" style={{color:T.textMuted,fontSize:14}}>Warmup · 15m</div>
                      </div>
                    </div>

                    {/* Each drill — full card style */}
                    {(p.drills||[]).map((d,i)=>{
                      const c=CAT[d.category]||CAT["Hitting"];
                      ...
                      return (
                        <div key={i} className="plan-drill-row">
                          <div className="plan-drill-time-row">
                            ...
                            <div className="plan-drill-time">{blk.start} – {blk.end}</div>
                          </div>
                          <div style={{paddingLeft:18}}>
                            <div className="plan-drill-name">{d.name}</div>
                            <div className="plan-drill-meta">
                              <CatChip cat={d.category} small/>
                              <span className="player-chip"><Ico name="users" size={11}/>{d.players} players</span>
                              <span className="dur-chip"><Ico name="clock" size={11}/>{d.duration||20}m</span>
                              {d.venue&&d.venue!=="Both"&&<VenueChip venue={d.venue} small/>}
                            ...
                            {d.notes&&(
                              <ul className="plan-drill-notes">
                                {d.notes.split("\n").filter(Boolean).map((n,j)=>(
                                  <li key={j}><span style={{color:c.text}}>· </span><span style={{color:T.noteText}}>{n}</span></li>
                                ))}
                              </ul>
                            )}
                            {d.video&&(
                    ...
                    <div className="plan-drill-row">
                      <div className="plan-drill-time-row">
                        <div className="plan-drill-dot" style={{background:T.steel}}/>
                        <div className="plan-drill-time">{schedule[schedule.length-1].start} – {schedule[schedule.length-1].end}</div>
                        <div className="plan-drill-name" style={{color:T.textMuted,fontSize:14}}>Cool Down · 15m</div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              return (
                <div key={d.id} className={`pick-item${sel?" picked":""}`} onClick={()=>toggleEPick(d)}>
                  ...
                  <div className="pick-info">
                    ...
                    <div className="pick-meta"><CatChip cat={d.category} small/><span className="dur-chip" style={{fontSize:11,padding:"2px 7px"}}>{d.duration||20}m</span></div>
                  </div>
