...
async function sbGet(table,userId){
  ...
  if(error||!Array.isArray(data))return[];return data.map(r=>r.data);}catch{return[];}}

async function sbUpsert(table,id,data,userId){
  try{await supabase.from(table).upsert({id,data,user_id:userId},{onConflict:"id"});}catch{}}

async function sbDelete(table,id){
  try{await supabase.from(table).delete().eq("id",id);}catch{}}

async function sbGetSingle(table,userId){
  try{const{data}=await supabase.from(table).select("id,data").eq("user_id",userId).limit(1).single();
  return data?data.data:{};}catch{return{};}}

async function sbSaveSingle(table,userId,payload){
  try{await supabase.from(table).upsert({id:userId,user_id:userId,data:payload},{onConflict:"id"});}catch{}}
...
function PracticeSchedule({plan,logoUrl,teamName}){
...
// ─── MAIN APP ─────────────────────────────────────────────────────────────────
export default function PracticePlanner({user,team:initialTeam,onTeamUpdate,onSignOut}){
  const today=new Date().toISOString().split("T")[0];
  const userId=user.id;
  const[team,setTeamLocal]=useState(initialTeam);
  const ROSTER=team?.roster||[];
  const logoUrl=team?.logo_url||"/KMBA-Panthers-Logo_U8_Tier_1.png";
  const teamName=team?.team_name||"My Team";
  const teamSub=team?.age_group&&team?.season?`${team.age_group} · ${team.season}`:"Practice Planner";
  function updateTeam(t){setTeamLocal(t);onTeamUpdate&&onTeamUpdate(t);}
  ...
  const spinner=(msg)=>(
    <div style={{minHeight:"100vh",background:P.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16}}>
      <img src={logoUrl} alt="PracticePro" style={{width:72,height:72,objectFit:"contain",animation:"pp-spin 2s linear infinite"}}/>
  ...
  async function saveDrill(){
    ...
    await sbUpsert("drills",dr.id,dr,userId);toast.show(editId?"Drill updated ✓":"Drill saved ✓");resetForm();
  }
  ...
  async function savePractice(){
    ...
    await sbUpsert("plans",plan.id,plan,userId);
  ...
  async function saveEditPlan(){
    ...
    setPlans(prev=>prev.map(p=>p.id===editPlan.id?updated:p));await sbUpsert("plans",updated.id,updated,userId);setEditPlan(null);toast.show("Practice updated ✓");
  }
  ...
  async function toggleAttendance(playerId){
    ...
    setAttendance(next);
    await sbSaveSingle("attendance",userId,next);
  }
  ...
  async function awardMvp(playerId){
    ...
    setMvpCounts(next);
    await sbSaveSingle("mvp",userId,next);
    setUndoMvp(playerId);
        ...
        {tab==="plans"&&(()=>{
          ...
          return(
            <>
              ...
              <PracticeSchedule key={selectedDate} plan={selectedPlan} logoUrl={logoUrl} teamName={teamName}/>
            </>
          );
        })()}
        ...
        {tab==="mvp"&&(()=>{
          ...
          return(<>
            <div style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between",marginBottom:12}}>
              ...
              {mvpSubTab==="mvp"&&<div className="mvp-season-chip">{team?.season||"2026"}</div>}
            </div>
            ...
            {mvpSubTab==="att"&&(<>
              ...
              <div style={{display:"flex",gap:8,marginBottom:12}}>
                <button className="btn btn-ghost btn-sm btn-full" onClick={async()=>{const next={...attendance,[attDate]:Object.fromEntries(ROSTER.map(p=>[p.id,true]))};setAttendance(next);await sbSaveSingle("attendance",userId,next);}}>Mark All Present</button>
                <button className="btn btn-ghost btn-sm btn-full" onClick={async()=>{const next={...attendance,[attDate]:{}};setAttendance(next);await sbSaveSingle("attendance",userId,next);}}>Clear All</button>
              </div>
